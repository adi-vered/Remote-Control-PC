...
def on_move(x, y, injected):
    protocol.send(my_socket, f'MOVE,{x},{y}'.encode())

def on_click(x, y, button, pressed, injected):
    button_clicked = 'LEFT' if button == mouse.Button.left else 'RIGHT'
    protocol.send(my_socket, f'{'PRESS' if pressed else 'RELEASE'},{button_clicked}'.encode())

def on_scroll(x, y, dx, dy, injected):
    protocol.send(my_socket, f'SCROLL,{dx},{dy}'.encode())
